...
import ScheduleBlockEmpty from "./ScheduleBlockEmpty";
...
export default function Week(props) {
    ...
    useEffect(() => {
        preloaderStart();
        axios.post(`/react/schedule/get-data/${type}/${week.id}/${model.id}`)
            .then((res) => {
                setData(res.data);
            })
        ...
        return Object.keys(data.times).map((key) => {
            ...
            return (
                <div className={'row'} key={key}>
                    ...
                    <TimeBlock start={time.timeFrom} end={time.timeTo} time={time} type={key} opacities={data.timesOpacityPercent}/>
                </div>
            );
        });
    };
