...
import {preloaderStart} from "../../../src/Preloader";
...
function ScheduleBlock(props) {
    ...
    useEffect(() => {
        let teacherPos = teacher.position;
        ...
        <div className={'col-2 block-schedule-container'}>
            <div className={'block-schedule-main'}>
                ...
                <div className={'block-additional'}>
                    <span className={'teacher group'}
                          onMouseEnter={ (e) => {showAdditionalInfo(e, 'teacher')} }
                          onMouseLeave={ (e) => {showAdditionalInfo(e,'teacher')} }
                          onClick={ () => {redirect(type === 'teacher' ? `/schedule/group/${party.id}` : `/schedule/teacher/${teacher.id}`)} }
                        >
                        {renderTeacherGroupBlock()}
                    </span>
                    <span className={`block cabinet type-${buildings[building.id]}`}
                          onMouseEnter={ (e) => {showAdditionalInfo(e, 'cabinet')} }
                          onMouseLeave={ (e) => {showAdditionalInfo(e,'cabinet')} }
                          onClick={ () => {redirect(type === 'cabinet' ? `/schedule/group/${party.id}` : `/schedule/cabinet/${cabinet.id}`)} }
                    >
                        {renderCabinetGroupBlock()}
                    </span>
                    <span className={`block lesson_type type-${lessonType.id}`} onMouseEnter={ (e) => {showAdditionalInfo(e, 'lesson_type')} } onMouseLeave={ (e) => {showAdditionalInfo(e,'lesson_type')} }>
                        {lessonType.name}
                    </span>
                </div>
            </div>
            <div className={'info teacher_info group_info'}><p>{renderTeacherGroupBlockFull()}</p></div>
            <div className={'info cabinet_info group_info'}><p>{renderCabinetGroupBlockFull()}</p></div>
            <div className={'info lesson_type_info'}><p>{lessonType.name_full}</p></div>
        </div>
    )
}
...
export default withRouter(connect(null, matchDispatchToProps)(ScheduleBlock));
